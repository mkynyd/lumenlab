...
import { regradeCommandSchema } from "@/lib/learning/server/input-schemas";
...
export async function POST(request: Request, { params }: RouteContext) {
  return learningRoute(async ({ userId }) => {
    const parsed = await parseStrictJson(request, regradeCommandSchema);
    if (!parsed.success) return parsed.response;
    const { id: projectId, goalId, evaluationId } = await params;
    const result = await learningService.regradeEvaluation({
      userId,
      projectId,
      goalId,
      evaluationId,
