...
describe("research model routing", () => {
  ...
  it("routes all default roles through the active DeepSeek Responses model", () => {
    expect(selectResearchModel("research.worker")).toMatchObject({ provider: "deepseek", model: "deepseek-v4-flash-vision-exp", source: "default" });
    expect(selectResearchModel("research.synthesizer")).toMatchObject({ provider: "deepseek", model: "deepseek-v4-flash-vision-exp", reasoningEffort: "max" });
  });
  ...
  it("allows active overrides, upgrades legacy IDs, and rejects unknown models", () => {
    ...
    expect(selectResearchModel("research.verifier")).toMatchObject({ provider: "deepseek", model: "deepseek-v4-flash-vision-exp", source: "environment" });
