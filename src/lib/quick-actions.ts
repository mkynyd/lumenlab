...
Mermaid 语法硬约束：
1. 必须输出 flowchart LR，不要输出 graph LR。
2. 只输出 flowchart LR、subgraph、direction、节点定义、箭头和 end；绝对不要输出 classDef、class、style、linkStyle 或 init 配置。
3. 节点标签只使用纯中文或英文文字；绝对禁止在标签内使用半角括号 ()、大括号 {}、尖括号 <>、脱字符 ^、竖线 |。
4. 所有节点标签必须用方括号 [标签] 或圆角方括号 (标签) 包裹，标签内容不能为空、不能只含空格，也不能只重复节点 ID（如禁止 A1[A1]）。
5. 每个节点标签必须是能独立表达含义的简短中文短语，让读者不看图外说明也能理解节点内容。
6. 每个节点定义和每条连线独占一行。
7. subgraph 只能作为分组容器；禁止把箭头连到 subgraph ID，例如禁止 A5 --> L2、L2 --> L3、C3 --> L4。
8. 跨分组依赖必须连接到具体节点，例如 A5 --> B1、B5 --> C1、C3 --> D1。
9. 优先控制节点数量和文字长度，让图表可阅读。
