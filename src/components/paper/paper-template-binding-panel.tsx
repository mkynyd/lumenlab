  ...
  return (
    <section className="mt-5 bg-[var(--color-panel)] px-5 py-5">
      ...
      <form className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)_auto]" onSubmit={submit}>
        ...
        <select value={selectedId} onChange={(event) => setSelectedId(event.target.value)} aria-label="选择可执行论文模板" className="min-h-9 min-w-0 rounded-[var(--radius-md)] bg-[var(--color-bg)] px-3 text-xs text-[var(--color-text-secondary)] outline-none focus:ring-1 focus:ring-[var(--color-accent)]">
          <option value="">{query.trim().length < 2 ? "先输入至少两个字符搜索学校" : templatesQuery.isPending ? "正在读取模板…" : options.length > 0 ? "选择学校模板" : "没有已固定的可执行模板"}</option>
          {options.map(({ template, variant, lockedVersion }) => <option key={variant.id} value={variant.id}>{template.university} · {template.degreeType ?? "学位未知"} · {template.year ?? "年份未知"} · {lockedVersion}</option>)}
        </select>
