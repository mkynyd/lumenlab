...
const NO_PAPER_WORKSPACE = "__no_paper_workspace__";
...
export function ResearchPaperTransferPanel({ runId, workspaceId }: { runId: string; workspaceId: string }) {
  ...
  return <section className="mt-5 bg-[var(--color-panel)] px-5 py-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold text-[var(--color-text-primary)]">发送到论文</h2><p className="mt-1 text-xs text-[var(--color-text-tertiary)]">Research Source、Claim、Evidence 与 Paper Reference 保持不同职责，发送只建立可追溯材料链接。</p></div><Send className="text-[var(--color-accent)]" width={19} height={19} /></div><div className="mt-4 grid gap-4 sm:grid-cols-3"><ToggleList title="Sources" label="来源" items={(candidates?.sources ?? []).map((item) => ({ id: item.id, label: item.title ?? item.canonicalKey }))} selected={sources} onToggle={(id) => toggle(setSources, sources, id)} /><ToggleList title="Claims" label="命题" items={(candidates?.claims ?? []).map((item) => ({ id: item.id, label: item.statement }))} selected={claims} onToggle={(id) => toggle(setClaims, claims, id)} /><ToggleList title="Evidence" label="证据" items={(candidates?.evidence ?? []).map((item) => ({ id: item.id, label: item.statement }))} selected={evidence} onToggle={(id) => toggle(setEvidence, evidence, id)} /></div><div className="mt-4 flex flex-wrap items-center gap-2"><Select value={paperWorkspaceId || NO_PAPER_WORKSPACE} onValueChange={(value) => setPaperWorkspaceId(value === NO_PAPER_WORKSPACE ? "" : value)}><SelectTrigger aria-label="选择论文工作区" className="min-h-9 min-w-52 bg-[var(--color-bg)] text-sm text-[var(--color-text-primary)]"><SelectValue placeholder="选择论文工作区" /></SelectTrigger><SelectContent position="popper" align="start"><SelectGroup><SelectLabel>论文工作区</SelectLabel><SelectItem value={NO_PAPER_WORKSPACE}>选择论文工作区</SelectItem>{papers.map((paper) => <SelectItem key={paper.id} value={paper.id}>{paper.name}</SelectItem>)}</SelectGroup></SelectContent></Select><Button type="button" variant="secondary" size="sm" onClick={() => void submit()} disabled={!paperWorkspaceId || (sources.size === 0 && claims.size === 0 && evidence.size === 0) || transfer.isPending}><Check width={14} height={14} />发送选中材料</Button>{transfer.isSuccess ? <span className="text-xs text-[var(--color-text-secondary)]">已建立 PaperResearchMaterial 链接</span> : null}</div></section>;
}
