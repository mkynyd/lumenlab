...
export function ResearchWorkspaceView({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    events.addEventListener("research", (event) => {
      try {
        const payload = JSON.parse((event as MessageEvent).data) as { message?: string; publicData?: { counters?: ResearchBudgetCounters } };
