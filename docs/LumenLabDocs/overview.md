...
LumenLab 是一个**项目化**的 AI 学习工作台。它把以下能力整合在同一个工作流中：

- 以项目为单位管理资料与上下文。
- 根据项目类型、上传资料和提问内容匹配快捷任务、Skill 与提示词。
- 受控的 Agent 模式，AI 调用工具前会经过策略引擎，重要操作会请求你确认。
...
### Skill Router 与受控 Agent

- Skill Router 会根据任务自动激活合适 Skill，也允许你手动切换或关闭。
- AI 可以在策略允许时调用工具，例如读取项目资料、搜索网页、保存成果、管理参考文献。
- 涉及保存、导出、删除等操作前，系统会按风险等级弹出审批卡片，由你决定允许或拒绝。

### 内置 Skills

- 提供 14 个 bundled Skill，新增 Deep Research 核心方法论，并覆盖论文阅读、论文写作、文献综述、图表规范、中文润色、考点提取、备考教练、代码阅读、PDF、Word、PPT、表格和苏格拉底式导师。
- 每个 Skill 相当于一个场景模板，会自动收窄 AI 可使用的工具范围。
- 你可以在输入工具栏手动切换 Skill，或关闭 Skill 回到通用对话模式。

### 内置 Tools

- 项目文件管理：列出、读取、删除项目中的资料。
- 成果管理：保存、列出、导出 DOCX。
- 网页获取：`web.search` 搜索公开网页，`web.fetch` 在 allowlist 范围内读取页面内容。
- 项目资料检索：基于项目资料进行关键词和语义搜索。
- 学术检索：搜索、读取和抓取 arXiv 论文页面。
- 参考文献：添加、列出、关联和格式化参考文献。
- Skill 激活：`skill.activate` 可根据已发现的 Skill 包切换详细工作指令。
...
- 未配置语义检索时自动降级为关键词检索。
- 基于项目文件、网页或 arXiv 论文生成的回答，会在消息底部展示来源 UI，方便你核对引用出处。
- 项目资料图谱按 topic / file / chunk 展示资料关系，帮助你检查知识结构和解析覆盖。
