...
| L4 外部 API 提示缓存 | DeepSeek prompt cache | 利用模型侧提示缓存，降低 token 费用与延迟 |
...
- Token 使用量（按 DeepSeek / MiniMax 拆分）。
- 导出缓存命中 / 未命中统计。
...
DeepSeek 支持 prompt cache。`src/app/api/chat/route.ts` 在构造消息列表时：
