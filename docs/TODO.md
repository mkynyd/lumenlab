...
- [ ] Materialize audited upstream files and run real sample/conformance compilation for A/B packs first; C/D records remain visible and enter the validation queue.
- [ ] Add the fully isolated CompileJob/Worker service using existing Artifact/Object Storage. The current first slice already has an ephemeral workspace, `shell: false`, `--no-shell-escape`, latexmk/BibTeX/Biber fallback, bounded timeout, asset/manifest source bundle, normalized node errors, PDF/source/SyncTeX upload and last-successful-PDF retention; network/filesystem/CPU/memory quotas and a separately deployed service remain intentionally deferred.
...
- [ ] Remaining before declaring the full roadmap complete: separately isolated compiler service with materialized pinned upstream template files and real A/B pack compilation, an interactive PDF SyncTeX viewer (download/mapping endpoint exists), AI-assisted ambiguous import classification, richer DOCX assets/footnotes/equations, and model-backed Planner/Evaluator/Verifier stages beyond the current role-routing seam.
- [x] Research role model routing and token/credit accounting now use the existing provider catalog, durable checkpoint, `calculateCredits` weights and public budget events; the remaining model-backed stage expansion is intentionally separate from the accounting seam.
