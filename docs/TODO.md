...
- [x] Add a repeatable pinned-upstream materializer that reads the real registry metadata, resolves an audited GitHub commit, normalizes a zip snapshot, stores it through the existing object-storage adapter and locks the snapshot into the Template Variant. The first real materialization is `tuna/thuthesis` at commit `fd3be474b1e66be85bcf286ea4be524b26c7e512` (269 files, 1,096,510 bytes); no upstream repository is vendored into Git.
