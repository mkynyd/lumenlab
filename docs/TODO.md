...
- [x] Add a repeatable pinned-upstream materializer that reads the real registry metadata, resolves an audited GitHub commit, normalizes a deterministic `normalized-v2` zip snapshot, stores it through the existing object-storage adapter and locks the snapshot into the Template Variant. The first real materialization is `tuna/thuthesis` at commit `fd3be474b1e66be85bcf286ea4be524b26c7e512` (269 files, 1,094,788 bytes); no upstream repository is vendored into Git.
- [x] Add a real sample/conformance validator that reads pinned snapshots, builds missing `.cls` files from pinned `.ins/.dtx` sources in the ephemeral workspace, enforces `--no-shell-escape`/`-norc`, emits PDF evidence and records `Verified` or `Needs Review` with normalized errors. The first 15 materialized Variants were compiled locally: 3 verified and 12 retained for review with concrete template-specific errors.
- [ ] Materialize the remaining audited A/B upstream packs and run real sample/conformance compilation for them; C/D records remain visible and enter the validation queue.
...
- [ ] Remaining before declaring the full roadmap complete: broaden pinned snapshot materialization and real A/B pack compilation, finish production container/cgroup isolation, an interactive PDF SyncTeX viewer (download/mapping endpoint exists), AI-assisted ambiguous import classification, richer DOCX assets/footnotes/equations, and real-provider end-to-end validation/repair coverage for all model-backed Research stages.
