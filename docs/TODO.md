...
- [x] Make Follow-up Run inheritance explicit: active Source Snapshots, Evidence and Claims are copied into new Run-owned records with origin provenance, pending re-verification and a new workspace-scoped Plan Version; the old Run/Plan/Report and its assets are not rewritten.
- [x] Make concurrent Researcher hard-budget accounting atomic: model/search/fetch/source operations reserve synchronously before async work, failed reads release source reservations, and the shared state cannot oversubscribe profile limits.
- [x] Make user-edited Claim reassessment executable: active Runs reuse the existing durable execution; terminal Runs create a deduplicated inherited Follow-up Run in `planning` and keep the normal plan-confirmation boundary before research resumes.
- [x] Enforce per-Question research/evaluation/replan ceilings in addition to Run-wide budgets; exhausted retryable Tasks now terminate with a public reason instead of entering a hot loop.
- [x] Make Evaluator stop metrics use current-pass Question statuses, unique Source identities, persisted quality dimensions and checkpointed new-Evidence information gain; remove the fixed conflict-coverage and non-empty-Evidence shortcuts.
...
- [x] Let users choose an independent or Project-backed Research Workspace and select Quick/Deep/Comprehensive at workspace/run creation; expose the selected context and budget profile in the real API path.
- [x] Add a public execution feed sourced from replayable Research SSE events, with current pending/running/retrying tasks, per-Question completion indicators and query/provider/source metadata; hidden reasoning remains excluded.
...
- [x] Add owner-scoped Document version history and recovery: historical versions are listed without exposing other users' documents, and recovery appends the selected immutable content as a new current version rather than rewriting history.
...
- [x] Make heading movement usable from the editor: the hover handle supports native drag-and-drop of a complete heading subtree, and Outline/compile-error selection scrolls to and highlights the corresponding Document node.
- [x] Preserve and edit rich inline nodes instead of flattening them to plain text: paragraphs, headings, quotes, abstract, acknowledgement and list items retain Text/Bold/Italic/Superscript/Subscript/InlineMath/Citation/CrossReference/Footnote nodes, with insertion controls for each supported inline kind.
- [x] Extend editor insertion and editing to schema-backed `Table`, `Bibliography`, `Appendix` and `PageBreak` blocks; bibliography IDs remain editable links to the separate Paper Reference panel, while appendix content continues in Document order.
- [x] Add an owner-scoped Paper image upload endpoint using the existing Object Storage/FileAsset seams; the editor can insert uploaded PNG/JPEG/GIF/WebP/SVG assets as Figure blocks and the existing Compile Worker validates and reads the stored Asset ID.
- [x] Normalize compile failures to a nearest Document `nodeId`: the renderer records both main/generated source lines, the Worker persists the mapped node on failed `PaperCompilation`, and the PDF rail exposes a direct “定位到 Document 块” action while retaining the normalized error code/message.
- [x] Make the editor's supported blocks truly editable: metadata/keywords, prose/quote, equation LaTeX/labels, figure asset metadata, table cells/rows/captions, lists and Raw LaTeX no longer render as placeholders.
...
- [x] Add a 1.2-second idle Document debounce that saves a new version and queues the background CompileJob; manual compile first persists an unsaved draft, and import structure confirmation blocks autosave until confirmed.
...
- [x] Make inline Citation editing choose an owned Paper Reference when available, preserve an explicit invalid-reference state, and warn when nested citations or Bibliography IDs are not present in the current Paper Workspace.
...
- [x] Add a real sample/conformance validator that reads pinned snapshots, builds missing `.cls` files from pinned `.ins/.dtx` sources in the ephemeral workspace, enforces `--no-shell-escape`/`-norc`, emits PDF evidence and records `Verified` or `Needs Review` with normalized errors. Successful validation now persists the PDF through the existing Object Storage adapter under a deterministic hash key; failed revalidation keeps the last successful Sample PDF reference while exposing the new error. The current local evidence covers all 164 materialized LaTeX/Overleaf variants: 43 verified PDFs and 121 Needs Review records; 18 Typst variants were explicitly skipped. Source-aware adapters now handle generic classes declared by source `.tex`, `\ProvidesExplClass`, pinned `.dtx` bootstrap for every declared output (`.cls/.def/.cbx/.bbx`), jobname-derived `.ins` installers such as HIT's `hithesis.ins`, Git-submodule-backed class files through temporary pinned checkouts and `normalized-v3-submodules` snapshots, nested class files mapped to the compile root, local preamble packages, comment-safe package discovery, common metadata/abstract conventions, JNU title/abstract/keyword conventions, SHU degree options, SCU cover/degree/abstract conventions, HFUT key-value metadata plus its missing `array` preamble dependency, Suda conditional biblatex detection, custom degree/title argument shapes, legacy `../Template/` runtime references and class-declared bibliography backends/styles. The local MacTeX `biber` executable is currently invalid (returns `lipo` usage), so affected packs are classified as `BIBLIOGRAPHY_BACKEND_UNAVAILABLE` rather than as template syntax failures; HUST additionally records its external `hustvisual.sty` dependency as a missing package.
...
- [ ] Resolve the remaining 14 unmaterialized A/B records. The latest retry on 2026-08-24 again covered all seven unique URLs/repositories behind the sourced records: three Overleaf pages still do not expose a safe, public, version-pinnable upstream repository (ShanghaiTech also failed page fetch), and four GitHub repositories still fail `git ls-remote`; one North Power record has no source URL and is therefore not retriable until registry metadata changes. Keep all of these visible as failed/unmaterialized and retryable; do not copy private Overleaf project data or invent a source snapshot. C/D records remain visible and enter the validation queue.
...
- [x] Upgrade deterministic LaTeX import to parse standard metadata, abstract/keywords, section hierarchy and labels, equations, tables, lists, quotes, page breaks, inline citation/cross-reference/footnote marks, `thebibliography` bibitems and known bibliography commands; persist parsed bibitems as Paper References and rewrite citation/bibliography keys to owned Reference IDs. Preserve custom macros/environments and standalone external-image Figure environments as low-confidence RawLaTeXBlock records instead of inventing invalid FileAsset IDs.
- [x] Add user-facing structure confirmation controls for low-confidence import blocks: paragraph, level-1/level-2 heading, quote, existing-asset Figure and Bibliography conversion preserve the imported content while keeping confirmation as the save/compile boundary.
- [x] Enforce the import confirmation boundary server-side: pending imports reject ordinary Document version writes and CompileJobs with a normalized `PaperServiceError`; only the owner-scoped confirmation transaction can use the bounded internal bypass, and the shared HTTP mapping returns a stable `409`.
- [x] Route Markdown through the existing unified/remark AST parser (GFM plus math), converting abstract/keywords, headings, inline emphasis/math, lists, blockquotes, tables, code/raw blocks and thematic breaks deterministically; invalid empty keyword nodes are never emitted into the strict Academic Document schema.
...
- [x] Connect Research cancellation to the existing durable `AgentExecutionStore.cancelOwned` path and expose owner-scoped cancel and immutable Follow-up actions in the Research Workspace; Follow-up creation selects the new Run while preserving the old Run/Plan/Report.
- [x] Re-ran the full gate after the template installer/HIT adapter slice: `302` test files / `1596` tests passed; targeted template/renderer tests were `2` files / `21` tests, and HIT's real local isolated-workspace validation produced a verified PDF. TypeScript, ESLint, Prisma validate, `prisma migrate status`, production build and `git diff --check` all passed. The build retains the known non-fatal NFT tracing warning from the dynamic `compile-worker` filesystem import. The previous Playwright smoke rendered `/research`, `/papers` and `/papers/typesetting` without framework overlays, and unauthenticated Research/Paper owner-scoped API calls returned the expected `401`. The credential-gated Provider integration test remains intentionally skipped without an E2E user/credentialed environment, and isolated Linux template validation remains blocked by the host disk capacity required for the `texlive-full` image, as recorded above.
- [x] Re-ran the full gate after the JNU/SHU/Suda/HFUT adapter slice: default `npm test` passed with `302` test files / `1601` tests; the targeted renderer/snapshot slice is `2` files / `26` tests. JNU/SHU/Suda each compile successfully in the local isolated validator, and both HFUT master/doctor variants produce verified PDFs. TypeScript, ESLint, Prisma validate, `prisma migrate status`, production build and `git diff --check` all passed. The build retains the known non-fatal NFT tracing warning; the Linux `texlive-full` image and credential-gated Provider E2E remain release-gate items.
- [x] Re-ran the full gate after the submodule materializer and SCU adapter slice: default `npm test` passed with `302` test files / `1602` tests; the targeted renderer/snapshot slice is `2` files / `27` tests. SCU bachelor/master/doctor compile with verified PDFs. TypeScript, ESLint, Prisma validate, `prisma migrate status`, production build and `git diff --check` all passed. The build retains the known non-fatal NFT tracing warning; Linux `texlive-full` and credential-gated Provider E2E remain release-gate items.
...
- [x] Follow-up runs now preserve prior research as copied, provenance-linked context and re-evaluate inherited Claims in the new Run; concurrent Researcher counters use bounded synchronous reservations for hard budget enforcement.
- [x] Claim editing now exposes an owner-scoped reassessment action with duplicate Follow-up protection; old immutable Reports are never rewritten.
- [x] Research Question attempt counters now actively bound each Question's loop for Quick/Deep/Comprehensive profiles.
- [x] Question `qualitySummary` now persists source/evidence/corroboration/diversity/conflict/coverage/recency dimensions, while public budget events expose computed information gain.
