...
- [ ] Remaining before declaring the full roadmap complete: retry the 25 inaccessible A/B GitHub sources and improve the 106 reviewed template packs, validate the materialized A/B packs inside the isolated Linux compiler image, add an interactive PDF SyncTeX viewer (download/mapping endpoint exists), AI-assisted ambiguous import classification, richer DOCX assets/footnotes/equations, and real-provider end-to-end validation/repair coverage for all model-backed Research stages.
