...
- [x] Re-ran the full gate after the current Research cancellation/Follow-up and Paper server-side import-boundary slice: `302` test files / `1594` tests passed; TypeScript, ESLint, Prisma validate, `prisma migrate status`, production build and `git diff --check` passed. The build retains the known non-fatal NFT tracing warning from the dynamic `compile-worker` filesystem import. Playwright smoke rendered `/research`, `/papers` and `/papers/typesetting` without framework overlays, and unauthenticated Research/Paper owner-scoped API calls returned the expected `401`. The credential-gated Provider integration test remains intentionally skipped without an E2E user/credentialed environment, and isolated Linux template validation remains blocked by the host disk capacity required for the `texlive-full` image, as recorded above.
