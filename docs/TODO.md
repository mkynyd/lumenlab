...
- [ ] Remaining before declaring the full roadmap complete: broaden pinned snapshot materialization and real A/B pack compilation, finish production container/cgroup isolation, an interactive PDF SyncTeX viewer (download/mapping endpoint exists), AI-assisted ambiguous import classification, richer DOCX assets/footnotes/equations, and real-provider end-to-end validation/repair coverage for the model-backed Research Worker/Evaluator/Verifier stages.
- [x] Research role model routing and token/credit accounting now use the existing provider catalog, durable checkpoint, `calculateCredits` weights and public budget events. Research Worker, Evaluator, Synthesizer and Verifier now call the existing Runtime through a structured-output seam with deterministic fallback; real-provider end-to-end validation remains a release-gate item.
