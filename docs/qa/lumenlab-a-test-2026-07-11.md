...
## 角色 4 补充回归：RAG 端点、工具标记与快捷任务（09:16–09:42 CST）

### 用户回报与根因

| 优先级 | 症状 | 根因 | 修复 |
|---|---|---|---|
| P1 | 文件解析时 `qwen3-vl-embedding` 连续返回 `InvalidParameter url error`，文本回退也失败 | `dashscope-sdk-official` 将新模型请求硬编码到旧 `multimodal-embedding/one-peace` 路由；项目厂家文档要求 `multimodal-embedding/multimodal-embedding` | 移除旧 SDK，按本地厂家文档直接调用 DashScope 原生 HTTP；增加 30 秒超时、响应校验与正确请求体测试 |
| P1 | 旧项目 247 个 chunk 无向量，影响向量召回 | 历史解析任务在端点错误期间已完成文本分块，但所有 embedding 写入失败 | 使用当前百炼凭证在线验证 1024 维返回，并批量补建“网络安全”项目 247/247 个向量；纯文本 chunk 每 10 条批处理 |
| P1 | 模型输出 `<tool_c`、`<inv`、`</invoke`、`</tool_calls` 等内部标记 | 流式模型可在标签名中间切块，旧清洗器只处理完整标签或完整单词前缀 | 清洗器识别任意 XML 工具标签前缀并循环剥离连续残片；数据库最终消息继续做二次清洗 |
| P1 | 个性化配置后默认快捷任务看似被覆盖，通用项目没有 Mermaid | 个性化批量保存误写 `isSystem: true`；前端只要收到服务端 actions 就整组替换默认集合；通用默认集合缺 Mermaid | 个性化任务改存为非系统任务；前端按标题合并内置集合并兼容旧错误数据；通用项目补“生成 Mermaid 逻辑图”，个性化任务独立折叠显示 |

### 真实回归证据

- 本地厂家文档 `docs/aliyun/aliyun-multimodal-embedding.md` 与实现一致：原生端点为 `.../multimodal-embedding/multimodal-embedding`，`parameters.dimension=1024`，纯文本不开启融合。
- 使用当前中央百炼凭证执行在线探针，成功返回 1024 维向量；未输出或记录密钥。
- 修复前“网络安全”项目向量统计为 `total=247, missing=247`；补建后为 `total=247, missing=0`。
- 浏览器中旧项目同时显示“总结要点 / 深入分析 / 格式化 / 生成 Mermaid 逻辑图”和“个性化任务 (5)”。
- 点击 Mermaid 快捷任务后，服务端确定性预取全部 4 份 PPT；最终回答生成可渲染 Mermaid、4 文件覆盖表，并显示 4 个项目资料来源。
- 最终页面不含 `<tool_c`、`<inv`、`</invoke`、`</tool_calls`，浏览器控制台 0 error / 0 warning。

### 补充回归结论

本次新增的三个 P1 回归问题均已修复，现有资料数据已恢复。最终未解决 P0/P1/P2 仍为 0，A 测放行结论保持不变。
