...
生产环境只允许通过自动化部署脚本发布（脚本为内部运维文件，不随公开仓库分发）。不要在服务器上手工执行 `git pull`、`npm install`、`npm run build`、复制 `.next` 或直接改写运行目录软链接；这些操作会绕过 CI 门禁、数据库快照、预检和自动回滚。

### 发布前提

- 只有用户明确要求部署时才执行生产发布；普通的提交或推送不自动扩大为部署授权。
- 待发布代码必须已经进入 `main` 并推送到 `origin/main`。
- 先记录待发布的完整 commit SHA，并始终显式传给部署脚本，避免等待 CI 期间 `main` 又前进。
...
- 本机需要可用的 GitHub 凭据（`gh auth token` 或 Git credential）和服务器 SSH 入口。
- 主工作树如有无关未提交或未跟踪文件，必须保留，不得为了部署顺手清理。

### 标准发布

等目标 commit 的 GitHub Actions 全部变绿后，通过部署脚本发布指定的完整 commit SHA。发布流程自动完成 CI 门禁校验、数据库快照、Prisma migration、生产构建、新 release 健康预检、原子切换与健康检查；切换后检查失败时自动回滚到上一 release，服务器仅保留当前版本与最近一个可回滚版本。脚本同时提供发布状态查看与回滚能力。
...
1. 修改前用 Grep 搜索该 class / token / 组件的所有使用方，列出受影响范围。
2. 样式被多处共享时，优先在调用点局部覆盖（如下一层 `className`），不要直接改共享定义；确需改共享定义的，逐一核对每个使用方的渲染效果。
3. 本项目 Tailwind 半径比例被重定义（`rounded-md`=10px、`rounded-lg`=14px），小尺寸元素上 `rounded-lg` 可能接近甚至等于正圆，修改圆角相关样式时按实际渲染尺寸核算。
4. 改完后用 Playwright 截图核对目标组件和所有共享使用方。
